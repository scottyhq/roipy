...
1)
import roipy as rp
rp.models.okada.benchmark()
2)
params = rp.models.examples.okada('point sill')
ux,uy,uy = rp.models.okada(*params)
...
# Autogenerated with SMOP version 0.22, then manually checked
# smop -v calc_okada.m -o calc_okada.py
...
def fBi(sig, eta, parvec, p, q):
    ...
    if indfix.any():
        ...
        # indfix=np.flatnonzero(abs(q) > epsn)
        indfix = (np.abs(q) > epsn)
        theta[indfix] = np.arctan(
            (sig[indfix] * eta[indfix]) / (q[indfix] * R[indfix]))
    else:
    ...
    if np.abs(cosd) < epsn:  # 90 degree dipping plane (e.g. perfec strike-strip fault)
        ...
        #                             (sig.*RX.*cosd) );
        I5 = a * 2.0 / cosd * \
            np.arctan((eta * (X + q * cosd) + X * RX * sind) / (sig * RX * cosd))
...
def calc_okada(U, x, y, nu, delta, d, length, W, fault_type, strike, tp):
    ...
    # NOTE: can benchmark against 1985 paper Table #2
    #
    '''
    pi = np.pi
    ...
    a = 1 - 2 * nu  # NOTE that mu/(lambda+mu) = 1-2*nu
...
def calc_okada_multimode(U, x, y, nu, delta, d, length, wid, strike, tp):
    ...
    ux1, uy1, uz1 = calc_okada(u_ds, x, y, nu, delta, d,
                               length, wid, 2, strike, tp)
    ux2, uy2, uz2 = calc_okada(u_t, x, y, nu, delta, d,
                               length, wid, 3, strike, tp)

    UX = ux + ux1 + ux2
    UY = uy + uy1 + uy2
    UZ = uz + uz1 + uz2

    # NOTE: add fault rake adjustment? (pointed out by F. Delgado)
    # U = UX*np.cos(rake) + UY*np.sin(rake) + UZ

    return UX, UY, UZ


def Emu2nu(E=75e9, mu=30e9):
    """
    Okada solution conveniently expressed in terms of Poisson Ratio, nu,
    so for convenience the function relating nu, E (young's modulus), and mu (shear modulus)
    """
    return E / (2 * mu) - 1
...
def benchmark(inc=23.0, ald=-77.0, wavelength=5.66):
    ...
    # Note 201 critical because includes exactly 0
    xx = np.linspace(-100, 100, 201) * 1e3
    ...
    ux, uy, uz = calc_okada(U, x, y, nu, delta, d, length,
                            width, fault_type, strike, tp)
    ...
    params = dict(xcen=xcen, ycen=ycen, U=U, d=d, nu=nu, delta=delta, strike=strike, length=length,
                  width=width, fault_type=fault_type, inc=inc, ald=ald, wavelength=wavelength)
...
def tinker(example, inc=23.0, ald=-77.0, wavelength=5.66):
    ...
    xcen, ycen, U, d, nu, delta, strike, length, width, fault_type = examples.okada(
        example)
...
def plot_components(x, y, ux, uy, uz, los, params, profile=False):
    ...
    ax1.quiver(x[::ny, ::ny], y[::nx, ::ny], ux[
               ::nx, ::ny], np.zeros_like(uy)[::nx, ::ny])
